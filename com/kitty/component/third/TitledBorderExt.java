 ...
 * @author Amy Fowler
 */
public class TitledBorderExt extends AbstractBorder {
     ...
     */
    public TitledBorderExt(String title) {
     ...
     */
    public TitledBorderExt(Border border) {
     ...
     * @param title  the title the border should display
     */
    public TitledBorderExt(Border border, String title) {
     ...
     */
    public TitledBorderExt(Border border, String title, int titleJustification, int titlePosition) {
    ...
    public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
        ...
        Rectangle grooveRect = new Rectangle(x + EDGE_SPACING, y + EDGE_SPACING, width
       ...
       // int stringWidth = SwingUtilities2.stringWidth(jc, fm, getTitle());
        ...
        case DEFAULT_POSITION:
            diff = Math.max(0, ((ascent / 2) + TEXT_SPACING) - EDGE_SPACING) - 5;
            grooveRect.y += diff;
            grooveRect.height -= diff;
        ...
        if (border != null) {
            if (((titlePos == TOP || titlePos == DEFAULT_POSITION) && (grooveRect.y > textLoc.y
                    - ascent))
                    || (titlePos == BOTTOM && (grooveRect.y + grooveRect.height < textLoc.y
                            + descent))) {
                ...
                if (computeIntersection(clipRect, x, y, textLoc.x - 1 - x, height)) {
                    g.setClip(clipRect);
                    border.paintBorder(c, g, grooveRect.x, grooveRect.y, grooveRect.width,
                            grooveRect.height);
                }
                ...
                if (titlePos == TOP || titlePos == DEFAULT_POSITION) {
                    ...
                    clipRect.setBounds(saveClip);
                    if (computeIntersection(clipRect, textLoc.x - 1, y, stringWidth + 2, textLoc.y
                            - ascent - y)) {
                        g.setClip(clipRect);
                        border.paintBorder(c, g, grooveRect.x, grooveRect.y, grooveRect.width,
                                grooveRect.height);
    ...
    public Dimension getMinimumSize(Component c) {
        ...
        case BELOW_BOTTOM:
            minSize.width = Math
                    .max(fm.stringWidth( getTitle()), minSize.width);
            break;
        case BELOW_TOP:
        case ABOVE_BOTTOM:
        case TOP:
